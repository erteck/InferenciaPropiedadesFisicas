"""
Generator for `Katachi_Entrenamiento_Taylor.ipynb`.

This script emits a self-contained Colab notebook that fine-tunes Katachi's
ResNet50 on the Taylor stellar-mass label (`LogMassTaylor` from
`color_morp.csv`), keeping the architecture and the training protocol
identical to yesterday's Pipe3D pilot so the *label-change* effect on
dispersion can be measured cleanly.

All mass columns are in log10(M/Msun) [dex]; residuals and RMSE / MAE
are therefore reported in dex.
"""
...
CELLS.append(md(
    ...
    "\n"
    "**Objetivo.** Repetir el entrenamiento imagen-only del baseline de "
    "Katachi (ResNet50 pre-entrenada en Pipe3D), pero cambiando el "
    "*target* de $\\log M_\\star^{\\text{Pipe3D}}$ (dex) a "
    "$\\log M_\\star^{\\text{Taylor}}$ (dex).\n"
    "\n"
    "**Sin mover arquitectura, transforms, optimizador ni splits.** Lo único "
    "que cambia entre el run de ayer (Pipe3D) y este notebook (Taylor) es la "
    "columna del target. Esto aísla el efecto de la etiqueta sobre la "
    "dispersión de los residuos.\n"
    "\n"
    "**Unidades.** Todas las masas están en dex — $\\log_{10}(M_\\star / M_\\odot)$. "
    "Los residuos, RMSE, MAE, $\\sigma$, MAD y $\\sigma_{\\text{MAD}}$ se "
    "reportan también en dex.\n"
    "\n"
    "**Diseño del experimento.**\n"
    "1. Reconstruir la cohorte común de 8 494 galaxias con máscaras de "
    "calidad idénticas a las del notebook de comparación de masas.\n"
    "2. Usar el split congelado Train/Val/Test = 6 510 / 1 149 / 835 "
    "(random_state = 42).\n"
    "3. Cargar el checkpoint pre-entrenado de Katachi "
    "(`Mass_ResNet50_log_t50_chain_90_10.pytorch`) como punto de partida.\n"
    "4. Evaluar ese checkpoint tal cual contra las etiquetas Taylor "
    "(baseline sin entrenar).\n"
    "5. Fine-tuning con `target_col = \"Taylor\"`, Adam lr = 1e-4, MSELoss, "
    "batch 32, 20 épocas máx., early stopping por RMSE de Validation.\n"
    "6. Reportar dispersión (RMSE, MAE, bias, σ, σ_MAD, Pearson, "
    "Spearman) en Validation y en Test.\n"
))
...
CELLS.append(md(
    ...
    "\n"
    "Se fija una semilla base y se identifican las versiones principales del "
    "entorno para poder repetir el experimento tal cual en otra sesión de "
    "Colab.\n"
))
...
# ---------------------------------------------------------------------------
# 2. Data — upload color_morp.csv, download scalars.cat
# ---------------------------------------------------------------------------
...
CELLS.append(md(
    ...
    "Dos fuentes:\n"
    "\n"
    "1. **`color_morp.csv`** (asesores) — trae `MANGAID`, `T_11`, `g_i`, "
    "`LogMassTaylor`, `LogMassNSA`. Se sube manualmente al entorno de "
    "Colab.\n"
    "2. **`scalars.cat`** (repositorio oficial de Katachi) — trae "
    "`mangaid`, `log_mstar` (Pipe3D, dex) y `split` "
    "(Train / Test originales). Se descarga por `wget`.\n"
))
...
CELLS.append(md(
    ...
    "\n"
    "Se aplican los mismos criterios que el notebook de comparación de "
    "masas (viernes pasado):\n"
    "\n"
    "- **Taylor válido**: `g_i` disponible, `g_i ≤ 2` y `LogMassTaylor` "
    "finita. El corte en `g_i ≤ 2` viene del asesor; se comprobó que "
    "elimina exclusivamente galaxias con colores irreales.\n"
    "- **NSA disponible**: `LogMassNSA` finita.\n"
    "- **Pipe3D disponible**: `log_mstar` finita.\n"
    "\n"
    "Las máscaras se crean como columnas booleanas — no se eliminan filas "
    "todavía.\n"
))
...
# ---------------------------------------------------------------------------
# 4. Merge → common cohort
# ---------------------------------------------------------------------------

CELLS.append(md(
    "## 4. Merge y construcción de la cohorte común\n"
    "\n"
    "Katachi es la tabla de referencia. Se hace un `left join` por "
    "`mangaid ↔ MANGAID` y se conserva solamente el subconjunto donde:\n"
    "\n"
    ...
    "\n"
    "Esa es exactamente la cohorte de 8 494 galaxias del notebook de "
    "comparación de masas. Como el corte es el mismo para las tres "
    "etiquetas, entrenar contra Pipe3D o contra Taylor se hace sobre "
...
CELLS.append(md(
    ...
    "\n"
    "- **Test** = Test original de Katachi (835 galaxias), reservado para la "
    "evaluación final.\n"
    "- **Train interno / Validation** = split aleatorio 85 / 15 del Train "
    "original con `random_state = 42`, sin estratificar por target — así "
    "los tres experimentos (Pipe3D, Taylor, NSA) usan exactamente los "
    "mismos IDs.\n"
    "\n"
...
CELLS.append(md(
    ...
    "\n"
    "`images.cat` (~1.9 GB) contiene un `DataFrame` con columnas "
    "`mangaid` e `images`. Cada imagen es un `PIL.Image` RGB de 256×256 "
    "px, es decir la representación tal cual la usó Katachi para entrenar."
    " No se le aplica ningún preprocesado adicional excepto `ToTensor()`, "
    "que escala los píxeles a [0, 1] — es exactamente el pipeline "
    "verificado en el notebook de comparación de masas.\n"
))
...
# ---------------------------------------------------------------------------
# 7. Taylor diagnostics
# ---------------------------------------------------------------------------

CELLS.append(md(
    "## 7. Diagnóstico previo del target Taylor\n"
    "\n"
    "Antes de entrenar, se compara la distribución de `Taylor` con la de "
    "`Pipe3D` sobre la **misma cohorte**. Esto es útil para interpretar "
    "el resultado: si la std intrínseca de `Taylor` es distinta a la de "
    "`Pipe3D`, el RMSE óptimo alcanzable también cambia — un modelo que "
    "solo predice la media tendría RMSE ≈ std del target.\n"
))
...
CELLS.append(md(
    ...
    "\n"
    "El `Dataset` está parametrizado por `target_col`: exactamente el "
    "mismo objeto se puede usar mañana para Pipe3D o NSA cambiando una "
    "sola cadena. Aquí lo instanciamos con `\"Taylor\"`.\n"
    "\n"
    "El único preprocesado es `ToTensor()` — coincide con el pipeline "
    "verificado en Katachi.\n"
))
...
CELLS.append(md(
    ...
    "- Se cargan los pesos de `Mass_ResNet50_log_t50_chain_90_10.pytorch` "
    "(entrenado por Katachi sobre Pipe3D). Se limpia el prefijo `module.` "
    "que introduce `DataParallel` y se carga con `strict=True`.\n"
    "\n"
    "Ese checkpoint es el punto de partida tanto del run de Pipe3D "
    "(ayer) como de este run de Taylor: fijar el mismo estado inicial es "
    "necesario para que la comparación de dispersión sea justa.\n"
))
...
# ---------------------------------------------------------------------------
# 10. Baseline eval on Validation (Taylor labels, untrained-for-Taylor model)
# ---------------------------------------------------------------------------

CELLS.append(md(
    "## 10. Baseline · checkpoint pre-entrenado evaluado en las etiquetas Taylor\n"
    "\n"
    "El checkpoint fue optimizado por Katachi contra Pipe3D. Al "
    "evaluarlo directamente contra Taylor obtenemos una **referencia de "
    "arranque**: cuánto se equivoca antes de hacer *cualquier* fine-tune "
    "sobre Taylor. Es el punto de partida contra el que se compara el "
    "modelo entrenado.\n"
))
...
CELLS.append(md(
    ...
    "\n"
    "- Punto de partida: `copy.deepcopy(model_base)` (checkpoint de "
    "Katachi).\n"
    "- Optimizador: `Adam(lr = 1e-4)` sobre **todos** los parámetros.\n"
    "- Pérdida: `MSELoss` en dex.\n"
    "- Batch 32, sin scheduler.\n"
    "- **20 épocas máximas** con *early stopping* sobre RMSE de Validation "
    "(paciencia 5). Al final se restaura el `state_dict` del mejor "
    "modelo por RMSE de Validation.\n"
    "\n"
    "**Ojo con el sesgo de inicialización.** El checkpoint fue optimizado "
    "por Katachi para Pipe3D; para Taylor el modelo tiene que *desaprender* "
    "ese sesgo. Con early stopping y paciencia 5 esperamos alcanzar el "
    "plateau de Validation, pero si la curva no aplana claramente hay que "
    "correr más épocas o partir de pesos de ImageNet.\n"
    "\n"
    "**Multi-semilla.** `SEEDS = [42]` corre una sola semilla. Cambiar a "
    "`[42, 7, 13]` (o similar) para tener una estimación de la variabilidad "
    "seed-to-seed. Se reporta el mejor modelo por RMSE de Validation "
    "entre todas las semillas.\n"
    "\n"
    "Además de la RMSE 'sucia' calculada durante el forward pass en "
    "modo `.train()` (que va cambiando con las actualizaciones de peso "
    "y con BatchNorm en modo por-batch), después de cada época hacemos "
    "una **evaluación limpia sobre Train con `.eval()`** — esa es la "
    "que aparece como `rmse_train_dex`.\n"
))
...
# ---------------------------------------------------------------------------
# 12. Final eval on Validation and Test
# ---------------------------------------------------------------------------

CELLS.append(md(
    "## 12. Evaluación final · Validation y Test\n"
    "\n"
    "Se evalúa el **mejor modelo por Validation RMSE** en las dos "
    "particiones. Test se toca una sola vez, al final, para el reporte.\n"
))
...
CELLS.append(md(
    ...
    "\n"
    "Para cada partición: (a) predicho vs. verdadero, (b) histograma de "
    "residuos, (c) residuo vs. masa verdadera. La curva y = x en (a) y "
    "la línea horizontal en (c) marcan el modelo ideal.\n"
))
...
CELLS.append(md(
    ...
    "- `predicciones_taylor_val.csv` y `predicciones_taylor_test.csv` — "
    "`mangaid`, target verdadero, predicción y residuo.\n"
    "- `metricas_taylor.csv` — la tabla comparativa completa (baseline "
    "vs. fine-tune Val vs. fine-tune Test).\n"
...
# ---------------------------------------------------------------------------
# 14.2 Cross-target comparison
# ---------------------------------------------------------------------------

CELLS.append(md(
    "## 14.2 Comparación cruzada Pipe3D · Taylor · NSA (opcional)\n"
    "\n"
    "Esta es la sección que responde directamente al asesor. Sube "
    "**cualquiera** de los siguientes archivos si ya los tienes:\n"
    "\n"
    "- `metricas_pipe3d.csv` (versión Pipe3D de este mismo notebook).\n"
    "- `metricas_nsa.csv` (versión NSA).\n"
    "\n"
    "Si todavía no existen, corre esta celda igualmente — solo se "
    "mostrará la fila de Taylor. Cuando termines los otros dos runs, "
    "vuelve, sube los archivos y regenera la tabla y la gráfica.\n"
))
...
# ---------------------------------------------------------------------------
# 15. Reading of results
# ---------------------------------------------------------------------------

# ---------------------------------------------------------------------------
# 16. Automatic analysis section (works for any target)
# ---------------------------------------------------------------------------

CELLS.append(md(
    "## 16. Análisis automático de resultados\n"
    "\n"
    "Esta sección se apoya en las predicciones que quedaron en memoria "
    "(`y_val_true/pred`, `y_test_true/pred`) y calcula:\n"
    "\n"
    "1. **Métricas centrales de dispersión** por partición.\n"
    "2. **Pendiente de residuo vs. masa verdadera** — mide *regression-"
    "to-the-mean* (el sesgo clásico de un regresor MSE que comprime "
    "hacia la media).\n"
    "3. **Fracción de outliers** ($|r - \\tilde{r}| > 2\\sigma_{\\rm MAD}$ "
    "y $> 3\\sigma_{\\rm MAD}$).\n"
    "4. **Residuos por bin de masa** — revela si el modelo se equivoca "
    "sistemáticamente en un régimen específico.\n"
    "5. **Efecto del fine-tune** — mejora relativa vs. baseline en RMSE "
    "y $\\sigma_{\\rm MAD}$.\n"
    "\n"
    "La celda produce números; la interpretación específica del run "
    "(qué significan para el asesor) queda en la sección 16.2 abajo.\n"
))
...
CELLS.append(md(
    ...
    "El experimento fija la arquitectura, el preprocesamiento y el "
    "split del entrenamiento original de Katachi, y solo cambia la "
    "columna del target: en vez de Pipe3D se entrena contra Taylor. "
    "Todo lo demás — la ResNet50, la inicialización desde el "
    "checkpoint pre-entrenado por Katachi, el transform, el "
    "optimizador y las particiones Train / Validation / Test — se "
    "conserva idéntico. Cualquier diferencia observable en las "
    "métricas se atribuye, por tanto, exclusivamente a la etiqueta.\n"
    "\n"
    "**La caída de RMSE está dominada por la corrección de un offset "
    "sistemático.** La RMSE de Validation pasa de 0.606 a 0.277 dex, "
    "una reducción del 55 %. Ese número tiende a leerse como si el "
    "modelo hubiese ganado capacidad discriminativa, pero el "
    "diagnóstico correcto es más matizado. El checkpoint pre-entrenado "
    "predice valores en la escala de Pipe3D, y Taylor está en promedio "
    "0.56 dex por debajo de Pipe3D en esta cohorte, así que el "
    "baseline traía un bias sistemático de +0.53 dex. Ese offset por "
    "sí solo aporta la mayor parte de la mejora observada; después del "
    "fine-tune el bias residual queda en +0.07 dex. Es decir, el "
    "modelo aprendió principalmente a restar el desplazamiento entre "
    "las dos definiciones de masa, no a extraer más información de las "
    "imágenes.\n"
    "\n"
    "**La mejora real en dispersión es modesta pero medible.** "
    "La métrica que aísla la ganancia genuina es la dispersión "
    "robusta $\\sigma_{\\rm MAD}$, porque es insensible al bias. En "
    "Validation baja de 0.271 a 0.217 dex, una reducción del 20 %; "
    "en Test baja de 0.251 a 0.215 dex, del 14 %. Este pedazo sí "
    "corresponde a señal visual que el modelo aprovechó una vez "
    "descontado el offset. En el mismo sentido apunta el coeficiente "
    "de Pearson, que apenas se mueve (0.933 → 0.938): el checkpoint "
    "pre-entrenado ya ordenaba correctamente las galaxias por masa, y "
    "el fine-tune ajustó la escala absoluta, no el ranking.\n"
    "\n"
    "**El entrenamiento se satura en las primeras épocas.** "
    "La curva de aprendizaje muestra que la mejor RMSE de Validation "
    "se alcanza en la época 3 de las 20 permitidas. A partir de la "
    "época 4 la métrica oscila entre 0.28 y 0.30 dex, y en la época "
    "8 aparece un spike simultáneo en train y val (0.33 y 0.36 dex) "
    "que muestra al optimizador desestabilizándose antes de que el "
    "early stopping (paciencia 5) detenga la corrida. Con LR = 1e-4 "
    "sobre todos los parámetros y arrancando desde un checkpoint "
    "prácticamente convergido, el presupuesto útil de entrenamiento "
    "es de unas pocas épocas; para entrenamientos más largos "
    "convendría bajar la LR o congelar el backbone.\n"
    "\n"
    "**Los residuos revelan un sesgo por régimen de masa.** "
    "Al analizar residuos por régimen aparece un patrón estructural "
    "claro: el modelo sobre-predice las galaxias más ligeras "
    "($M_\\star < 9.5$ dex) y sub-predice las más masivas "
    "($M_\\star > 11$ dex), con pendiente residuo-vs-verdadero de "
    "aproximadamente $-0.20$ dex/dex. Eso significa que el bias medio "
    "total de $+0.07$ dex no es un desplazamiento uniforme sino una "
    "compensación entre residuos positivos en el extremo bajo y "
    "negativos en el extremo alto. Es el artefacto clásico de un "
    "regresor entrenado con pérdida cuadrática cuando la señal visual "
    "disponible es limitada: la solución de menor pérdida promedio "
    "comprime las predicciones hacia la media de la distribución. "
    "Fuera de este sesgo estructural, el histograma de residuos "
    "muestra un core gaussiano limpio con $\\sigma \\approx 0.22$ dex "
    "y solo un puñado de outliers extremos (aproximadamente el 1 % "
    "del total), típicamente en galaxias masivas sub-representadas en "
    "Train.\n"
    "\n"
    "**El modelo llega cerca del piso físico definido por la propia "
    "etiqueta.** El contexto físico fija un límite natural a lo que "
    "se puede alcanzar con imágenes solas. Sobre la misma cohorte, "
    "el desacuerdo intrínseco entre Taylor y Pipe3D — dos estimadores "
    "independientes de la masa estelar — tiene "
    "$\\sigma_{\\rm MAD} = 0.176$ dex. Ese número refleja la "
    "incertidumbre inherente cuando se comparan dos métodos que miden "
    "la misma cantidad por caminos distintos. Nuestro modelo alcanza "
    "0.22 dex, apenas 0.04 dex por encima. La mayor parte del error "
    "residual, entonces, ya no proviene de una deficiencia visual del "
    "modelo, sino de dispersión que ninguna imagen RGB puede resolver "
    "porque no está en los pixeles — está en la propia ecuación "
    "fotométrica de Taylor (color $g - i$ y magnitud absoluta). Para "
    "bajar de ese piso habría que darle al modelo esas variables "
    "tabulares directamente.\n"
    "\n"
    "**Validation y Test son consistentes.** "
    "Las dos particiones dan RMSE parecidos (0.277 dex en Val, 0.258 "
    "en Test). Test salió ligeramente mejor, lo cual es aceptable "
    "considerando que se aisló al inicio y no participó en la "
    "selección del modelo, y que la diferencia (~7 %) está dentro del "
    "margen que dan submuestras de 1 149 vs 835 galaxias.\n"
    "\n"
    "**Alcance del resultado.** "
    "Con una sola corrida no se puede afirmar todavía si Taylor es "
    "una etiqueta mejor o peor que Pipe3D o NSA como target de "
    "entrenamiento; para eso hace falta correr las otras dos versiones "
    "del notebook cambiando `TARGET_COL` y comparar "
    "$\\sigma_{\\rm MAD}$ lado a lado. Con la evidencia actual lo "
    "que sí queda establecido es que el modelo llegó cerca del piso "
    "físico intrínseco de la propia etiqueta y que la ganancia "
    "efectiva — descontando la corrección de escala — fue moderada, "
    "del orden del 20 %.\n"
    "\n"
    "### 16.4 Notas técnicas\n"
    "\n"
    "Todas las métricas están en dex, la unidad natural de "
    "$\\log_{10}(M_\\star/M_\\odot)$. La cohorte (8 494 galaxias) y el "
    "split 6 510 / 1 149 / 835 vienen congelados del notebook de "
    "comparación de masas y se definen como la intersección de "
    "Taylor válido + NSA finito + Pipe3D finito para que los tres "
    "experimentos usen exactamente las mismas galaxias. El modelo se "
    "seleccionó únicamente por RMSE de Validation y Test se evaluó "
    "una sola vez al final. La corrida se hizo con una única semilla "
    "(SEED_BASE = 42); si se necesita una banda de incertidumbre, "
    "poner `SEEDS = [42, 7, 13]` en §11 y volver a correr — la "
    "variabilidad seed-to-seed en modelos de este tamaño ronda "
    "0.005–0.02 dex.\n"
))
...
out_path = Path(__file__).parent / "Katachi_Entrenamiento_Taylor.ipynb"
